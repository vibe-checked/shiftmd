...
const FTE_OPTIONS = [0.5, 0.6, 0.8, 1.0];
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <View style={styles.header}>
        <View>
          ...
          <Text style={styles.subtitle}>
            {data.physicians.length} on the rotation
          </Text>
        </View>
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        {data.physicians.length === 0 ? (
          <>
            <EmptyState
              ...
              subtitle="Add the doctors on your rotation, or load a sample roster of 10 to explore the app."
            />
        ...
        {data.physicians.length > 0 && (
          <Text style={styles.hint}>Tap a physician to edit their name or FTE.</Text>
        )}
      </ScrollView>
      ...
      <Modal visible={modal} animationType="slide" transparent onRequestClose={() => setModal(false)}>
        ...
        >
          ...
          <View style={styles.sheet}>
            ...
            <Text style={styles.fieldLabel}>Schedule load (FTE)</Text>
            <View style={styles.fteRow}>
              {FTE_OPTIONS.map((opt) => (
                <Pressable
                  key={opt}
                  style={[styles.fteChip, fte === opt && styles.fteChipActive]}
                  onPress={() => setFte(opt)}
                >
                  <Text style={[styles.fteText, fte === opt && styles.fteTextActive]}>
                    {opt === 1 ? 'Full' : `${opt}×`}
                  </Text>
                </Pressable>
              ))}
...
const styles = StyleSheet.create({
  ...
  fteRow: { flexDirection: 'row', gap: 10 },
  fteChip: {
    flex: 1,
    height: 46,
    borderRadius: theme.radius.md,
  ...
  fteChipActive: { backgroundColor: theme.colors.primarySoft, borderColor: theme.colors.primary },
  fteText: { fontSize: theme.font.body, fontWeight: '700', color: theme.colors.textMuted },
  fteTextActive: { color: theme.colors.primary },
});
